...
import './adminUsers.css'; // Importa los estilos
import Logo from './imagenesjp/imagenes/Logo.png'; //Importar logo del encabezado
...
function AdminUsuarios() { //componente principal
  ...
  useEffect(() => {
    fetch('http://localhost/barberiaApp/php/usuarios.php')//Trae la lista de usuarios desde esta ubicación
  ...
  fetch('http://localhost/barberiaApp/php/usuarios.php', {//Peticiones HTTP desde JavaScript
    ...
    .then(data => {
      if (data.success) {
        ...
        fetch('http://localhost/barberiaApp/php/usuarios.php')
          .then(res => res.json())
          .then(data => {
            setUsuarios(data);
            cerrarModal();
    ...
    //New
    .finally(() => {
    cerrarModal(); // ← Aquí se cierra el modal pase lo que pase
  });
};
    ...
    <>{/**Vista del componente ↓ */}
      <header>
        <div className="logo-titulo">
          <img src={Logo} alt="Logo JP Systems B&S" className="logo" />
